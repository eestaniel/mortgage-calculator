...
export const formatNumber = (input: string, { numberType = 'int' }: FormatNumberOptions = {}): string => {
...
export const formatForDisplay = (input: string, numberType: 'int' | 'float'): string => {
    if (numberType === 'float') {
        if (!input.includes('.')) {
            return input + '.00';
